...
def snake_energy(flattened_pts, distance, alpha, beta):
    """
    ...
    derived from the supplied distance image; and internal energy, which is computed
    ...
    This follows the "Snakes" formulation in Prince's Computer Vision book, page 389.
    ...
    Args:
        flattened_pts ((2*N,)-shaped numpy array): A flattened list of the contour 
            ...
            of points.
        distance (2D numpy array): A distance image. Typically this is the distance 
            transform from prominent features, e.g. edges. It can be any image that
            has smooth gradients and a minimum at "interesting" features.
    ...
    dist_vals = ndimage.interpolation.map_coordinates(distance, [pts[:,0], pts[:,1]], order=1)
...
#    curvature_energy = np.sum(curvature)
    curvature_energy = np.sum(curvature**2)

    return external_energy + alpha*spacing_energy + beta*curvature_energy
    


def fit_snake(pts, distance_im, alpha=0.1, beta=0.05, point_plot=None):
    """
    ...
    # optimize
    cost_function = partial(snake_energy, alpha=alpha, beta=beta, distance=distance_im)
    options = {'disp':False}  # 'maxiter':int
    options['maxiter'] = 60  # FIXME: how to check convergence?
    method = 'CG'  # 'BFGS', 'CG', or 'Powell'. 'Nelder-Mead' has very slow convergence
